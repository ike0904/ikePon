...
- 最小サイズは 320×180 です。ウィンドウの縦横比は、 16:9 で固定です。
...
| 項目 | 説明 | デフォルト |
|:---|:---|:---|
| フェードアウト時間 | パッド音声・映像のフェードアウト、フェーダーのフェード移動およびALL FADE のフェード秒数（共通） | 2.0 秒 |
...
パッドを叩くと、キーボードのパッドキーを押したときと同じ動作をします。ベロシティ（強弱）は音量に影響しません。叩いた瞬間にトリガーが発火します。

#### PAD BANK B：グローバル操作 ＆ バンク切り替え（ノート 52〜67）

```
[64:BANK A  ][65:BANK B  ][66:BANK C  ][67:BANK D  ]  ← 上段（常用バンク）
[60:BANK E  ][61:BANK F  ][62:BANK G  ][63:BANK H  ]
[56:FULL SCR][57:DISPLAY ][58:（予備） ][59:（予備） ]
...
| FULL SCR（56） | 動画ウィンドウの全画面トグル |
| DISPLAY（57） | 動画ウィンドウの表示／非表示トグル |
...
CC の値（0〜127）がそのままフェーダー値に反映されます。
