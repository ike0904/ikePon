...
**ヒント：** `.ikp` ファイルをメインウィンドウにドラッグ＆ドロップして開くことができます。また、`.ikp` ファイルをダブルクリックすると ikePon が起動してプロジェクトを直接読み込みます。
...
│████████████████████ プログレスバー ████│  ← クリックでシーク（最下 10px）
└────────────────────────────────────────┘
```

カテゴリバッジの右の **●** は再生中タップ設定を示します：
...
| ループ開始位置を現在位置に設定 | 現在の再生位置をループ折り返し先として登録（MOV・Loop 設定時のみ） |
...
| 設定 | MOVIE / BGM | SE |
|:---|:---|:---|
| フェードアウト（デフォルト） | ロングフェードアウト停止 | 最初から再再生 |
| カットアウト | 即座に停止 | 即座に停止 |
| 一時停止・再開 | 一時停止 → 再押しで再開 | （選択不可） |
...
再生中の全パッドをロングフェードアウトで停止します。再生中のパッドがない場合は何もしません。

### 6.3 PAUSE（緑ボタン / `Enter`）

MOVIE・BGM カテゴリの全再生中パッドを一時停止します（SE は対象外）。

- 一時停止中：PAUSEボタンのボーダーが黄色（静止）。一時停止中の各パッドがじんわり点滅
...
映像が再生されていない間、設定したスタンバイ画像を表示できます（設定 → スタンバイ画像）。未設定の場合は黒画面になります。

### 9.3 黒画面検出

DISPLAY ON 中に映像もスタンバイ画像も表示されていない状態（黒画面）を検出した場合、インフォメーションエリアにオレンジ色の警告が表示されます。映像が復帰すると自動消去されます。
