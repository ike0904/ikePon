...
public sealed class RelocateController
{
        ...
        Func<string, Task<string?>> promptUserForFile)
    {
        ...
        foreach (var hintDir in hintDirs)
        {
            ...
            var index = await Task.Run(() => BuildFileIndex(hintDir));
        ...
        if (unresolved.Count > 0)
        {
            ...
            if (selected != null && File.Exists(selected))
            {
                ...
                {
                    var index = await Task.Run(() => BuildFileIndex(newHintDir));
    ...
    private static Dictionary<string, List<string>> BuildFileIndex(string rootDir)
    {
