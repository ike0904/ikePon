...
・v1.5.1：フェードアウト中のタップ禁止（カットアウト・ALL CUTは除外）。
  SEは通常再タップ（最初から再生）。
  映像ディスプレイ右クリックでウィンドウサイズをデフォルト（1280×720）にリセット（FULL SCR時は無効）。

・v1.5.2：黒画面警告は映像/画像パッドが再生中のときのみ表示（何も再生していない時は出さない）。
  カットアウト時はスタンバイレイヤーを即時表示（opacity=1）してVLC白地フラッシュを防止。
  ShowStandby(immediate:true/false) で制御。
