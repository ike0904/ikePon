...
・v1.5.3：フェードアウト完了時もスタンバイを opacity=1 で即時表示（フェードインなし）。
  【根本原因】FadeTimer_Tick完了後、オーバーレイが閉じる前にスタンバイ opacity=0→1 のフェードインを
  開始していたため、オーバーレイが消えた瞬間にVLC白地（一時停止中は即時白化）が透けていた。
  修正：オーバーレイが覆っている間にスタンバイを opacity=1 で準備し、オーバーレイを閉じるだけにした。

・v1.5.4：フェードオーバーレイをコンテンツ領域のみに限定（タイトルバーを覆わない）。
  Content.PointToScreen(0,0) でコンテンツ領域の左上座標を取得しオーバーレイを配置。
  ALL FADEやPAUSEボタン有効中にパッド操作が行われた場合の挙動を統一。
  パッドタップ時にグローバル状態を即座に解除し、各パッド個別のFADE/PAUSE状態へ切り替える。
  フェードアウト中のパッドはそのまま継続（再起動しない）。
  再生開始パッドと同カテゴリのフェード中・一時停止中パッドは即座に停止。
