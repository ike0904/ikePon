...
<p style="font-size: 11pt; color: #888; margin: 0 0 32px;">v1.7.0</p>
...
| カットアウト | 即座に停止 |
| 一時停止／再開 | 一時停止・再開（SE カテゴリは無効）。最終フレーム停止中に選択するとフリーズを解除してアイドルに戻します（再生はパッドをタップ） |
...
同一パッドへの連打（デフォルト 100 ms 以内）は無視されます（設定で変更可）。
...
動画ウィンドウを表示・非表示でトグルします。操作後 1 秒間グレーアウトし、連打を防止します。LOCK ON 時は無効です。
...
- ウィンドウのリサイズ時に **16:9** のアスペクト比を自動維持します
...
映像再生中にウィンドウの端をドラッグしてサイズを変更すると、一瞬白い背景が見えることがあります。映像エンジン（LibVLC）の D3D11 バッファ更新タイミングによるもので、仕様の範囲です。イベント本番中にウィンドウサイズを変更することは通常ないため、実用上の影響はありません。
...
| 連打防止時間 | 同一パッドの連打を無視する時間 | 100 ms |
| プリロード上限 | プリロードする最大ファイル長（変更後は再起動が必要） | 10 秒 |

**注意：** 言語・プリロード上限を変更して OK を押すと、再起動確認が表示されます。`Y` で即座にアプリを再起動します。
...
| バージョン | 日付 | 内容 |
|:---|:------|:--------------------|
| v1.7.0 | 2026-07-24 | 動画ウィンドウのリサイズ時に 16:9 アスペクト比を自動維持。<br>連打防止時間のデフォルトを 500 ms から 100 ms に変更。<br>設定ダイアログの「WASAPI レイテンシ」項目を削除（内部自動設定に変更）。<br>MOV「最終フレームで停止」状態の操作を改善：右クリック「フェードアウト」「カットアウト」が正しく動作するよう修正、右クリック「一時停止/再開」でフリーズ解除が可能に、ALL CUT でフリーズ確実解除。<br>DISPLAY OFF→ON 時の映像・音声の復元安定性を向上。<br>その他安定性・信頼性の向上。 |
| v1.6.0 | 2026-07-14 | 映像ウィンドウのレターボックス・ピラーボックス白フラッシュを根本修正（VideoView 常時表示化）。<br>DISPLAY ON 時に映像が白くフラッシュする問題を修正。<br>ループ再生・音声同期の安定性向上。<br>DISPLAY OFF 状態で動画パッドをトリガーした際の音声即時再生を修正。<br>カットアウト実行時は連打防止（インターロック）の対象外に変更。<br>DISPLAY は毎回 OFF 状態で起動するよう変更（前回状態の引き継ぎを廃止）。<br>アプリ終了確認をポップアップからインフォメーションエリア表示に変更。<br>二重起動防止機能を追加。<br>バンク詳細設定ダイアログのエラーを修正。<br>映像フェードアウト中に DISPLAY / FULL SCR を操作すると映像が止まらなくなる問題を修正。<br>DISPLAY OFF 中にフェードアウト完了 → DISPLAY ON した際に映像が止まらなくなる問題を修正。<br>ALL FADE または PAUSE 中にパッドをタップした際、有効状態を即座に解除するよう変更。<br>DISPLAY ボタンの操作後グレーアウト時間を 1 秒に短縮。 |
...
| Pause/Resume | Pause or resume (SE not available). When selected on a frozen pad, clears the freeze and returns to idle (tap the pad to replay) |
...
Rapid re-triggering of the same pad (within 100 ms by default) is ignored. This duration can be changed in Settings.
...
Shows or hides the video window. Grays out for 1 second after use. Disabled when LOCK is ON.
...
- The window automatically maintains a **16:9** aspect ratio when resized
...
When dragging the window border to resize while video is playing, a brief white background may flash. This is caused by the video engine (LibVLC) D3D11 buffer update timing and is within expected behavior. Since window resizing is not typically performed during a live event, this has no practical impact.
...
| Rapid Fire Prevention | Ignore interval for repeated presses on the same pad | 100 ms |
| Preload Limit | Maximum file length to preload (restart required after change) | 10 sec |

**Note:** If you change the Language or Preload Limit and press OK, a restart confirmation appears. Press `Y` to restart the app immediately.
...
| Version | Date | Notes |
|:---|:------|:--------------------|
| v1.7.0 | 2026-07-24 | Video window now automatically maintains 16:9 aspect ratio when resized.<br>Changed default rapid-fire prevention time from 500 ms to 100 ms.<br>Removed WASAPI Latency setting from the Settings dialog (now auto-configured internally).<br>Improved MOV "Freeze Last Frame" operation: Fade Out and Cut Out in the right-click menu now work correctly; Pause/Resume is now available to clear the freeze state; ALL CUT reliably clears the freeze state.<br>Improved video/audio restoration when DISPLAY is turned back ON.<br>General stability improvements. |
| v1.6.0 | 2026-07-14 | Fundamentally fixed letterbox/pillarbox white flash (VideoView always-on approach).<br>Fixed: white flash when DISPLAY is turned ON while video is playing.<br>Improved loop playback and audio sync stability.<br>Fixed: audio now plays immediately when a video pad is triggered while DISPLAY is OFF.<br>Cut Out actions are now exempt from interlock (rapid-fire prevention).<br>DISPLAY always starts in OFF state (previous state no longer carried over).<br>App-quit confirmation moved from popup dialog to info area.<br>Added double-launch prevention.<br>Fixed: Bank Details dialog crash.<br>Fixed: video continuing to play when DISPLAY or FULL SCR is operated during a fade-out.<br>Fixed: video failing to stop when DISPLAY is turned ON before a fade-out completes.<br>ALL FADE and PAUSE active state is now immediately cancelled when a pad is tapped.<br>DISPLAY button gray-out time reduced to 1 second. |
